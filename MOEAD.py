...
class EnergyParams:
    ...
    max_rounds: int = 10  # giữ lại để không phá interface cũ, nhưng không dùng để dừng nữa
...
class MOEADMultiAUV:
    def __init__(self, coords: np.ndarray, visit_ids: List[Any], params: MOEADParams,
        ...
        self.weights = self._generate_weights(params.population_size, 4)
        self.neighbors = self._build_neighborhoods(self.weights, params.neighborhood_size)
    ...
    def _generate_weights(self, N, m):
        """
        Sinh weight vectors theo simplex-lattice thay vì Dirichlet random.
        Mục tiêu:
        - phủ đều không gian trọng số hơn
        - ổn định hơn giữa các lần chạy
        - vẫn đảm bảo đủ đúng population_size

        Với m != 4 sẽ fallback về Dirichlet để giữ tính tổng quát.
        """
        if N <= 0:
            return np.empty((0, m), dtype=np.float64)

        if m != 4:
            return np.random.dirichlet(np.ones(m), size=N).astype(np.float64)

        # Chọn H sao cho số điểm lattice >= N và gần N nhất có thể.
        # Số điểm trên simplex 4 chiều: C(H+3, 3)
        H = 1
        while math.comb(H + 3, 3) < N:
            H += 1

        lattice = []
        for a in range(H + 1):
            for b in range(H + 1 - a):
                for c in range(H + 1 - a - b):
                    d = H - a - b - c
                    lattice.append([a / H, b / H, c / H, d / H])

        weights = np.array(lattice, dtype=np.float64)

        # Tránh các weight vector trùng nhau do xử lý sau này.
        if len(weights) > 1:
            weights = np.unique(weights, axis=0)

        # Nếu số lượng lớn hơn N, lấy đều trên toàn bộ tập lattice.
        if len(weights) > N:
            idx = np.linspace(0, len(weights) - 1, N, dtype=int)
            weights = weights[idx]
        elif len(weights) < N:
            extra = np.random.dirichlet(np.ones(m), size=N - len(weights)).astype(np.float64)
            weights = np.vstack([weights, extra])

        # Chuẩn hóa lần cuối để tổng mỗi vector đúng bằng 1.
        denom = np.sum(weights, axis=1, keepdims=True)
        weights = weights / np.maximum(denom, 1e-12)
        return weights.astype(np.float64)

    def _build_neighborhoods(self, weights, T):
        d = np.linalg.norm(weights[:, None, :] - weights[None, :, :], axis=2)
    ...
    def solve(self):
        population = [self._random_solution() for _ in range(self.params.population_size)]
        F = np.array([self.evaluate(sol) for sol in population])
        z = np.min(F, axis=0).copy()

        archive = []
        archive_f_rows: List[np.ndarray] = []
        history = []

        weights = self.weights
        neighbors = self.neighbors
        pop_size = self.params.population_size
        _rr = self.rng.randrange
        T_nbr = neighbors.shape[1]
        ...
        for gen in range(self.params.generations):
            ...
            for i in range(pop_size):
                ...
                child = self._mutate(
                    self._crossover(population[int(nbrs[_i])], population[int(nbrs[_j])])
                )
                f_child = self.evaluate(child)

                np.minimum(z, f_child, out=z)
                ...
                for j in nbrs:
                    ...
                    if t_new <= t_old:
                        population[j] = {"perm": child["perm"][:], "cuts": child["cuts"][:]}
                        F[j] = f_child.copy()

                pending_children.append(child)
                pending_F.append(f_child)

            for child, f_child in zip(pending_children, pending_F):
                if not archive_f_rows:
                    archive.append(child)
                    archive_f_rows.append(f_child)
                    continue

                arr = np.array(archive_f_rows, dtype=np.float64)

                dominated_mask = (
                    np.all(arr >= f_child, axis=1) &
                    np.any(arr > f_child, axis=1)
                )
                child_dominated = np.any(
                    np.all(arr <= f_child, axis=1) &
                    np.any(arr < f_child, axis=1)
                )
                ...
                if not child_dominated:
                    ...
                    archive = [a for a, k in zip(archive, keep_mask) if k]
                    archive_f_rows = [r for r, k in zip(archive_f_rows, keep_mask) if k]
                    archive.append(child)
                    archive_f_rows.append(f_child)
        ...
        archive_f_list = list(archive_f_rows)
...
def run_fixed_cluster_hp_moead(
    ...
    result = {
        "algorithm": "MOEAD_FIXED_CLUSTERS_HP_SEQUENTIAL_UPLOAD_OPTIMIZED_V2_VISUAL",
...
def run_folder_moead(
    input_folder="/kaggle/input/datasets/hoandd2004/nodes-data",
    output_main_folder="/kaggle/working/results_history_fixed_hp_seq_upload",
    num_auvs=3,
    space_size=400,
    r_sen=60,
    max_cluster_size=25,
    ...
    for filename in files:
        ...
        output_path = os.path.join(file_output_folder, f"moead_fixed_hp_seq_upload_{base_name}.json")
    ...
    summary = {
        "algorithms": ["MOEAD_FIXED_CLUSTERS_HP_SEQUENTIAL_UPLOAD_OPTIMIZED_V2_VISUAL"],
